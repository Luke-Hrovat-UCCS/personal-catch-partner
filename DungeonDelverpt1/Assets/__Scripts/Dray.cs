...
{                             // b
    ...
    {
        if (isControlled) return;
        Item pup = colld.GetComponent<Item>();                            // b
        if (pup == null) return;

        switch (pup.itemType)
        {
            case Item.eType.health:
    ...
    static public int NUM_GOLD { get { return S.gold; } }
